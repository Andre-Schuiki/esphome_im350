...
namespace esphome {
    namespace iskra_am550 {
 
        static const char *TAG = "iskra_am550.sensor";
        byte message[121];
            ...
            if (use_test_data_ == true) {
                ...
                if (message[0] == 0x7E) { // No timeout
                    ...
                    if (message[1] == 0xA0) { // No timeout
                        cnt++;
                        while ( cnt < 121 && (millis() - act_time < max_wait_time_for_reading_data_) ) {
        ...
        ESP_LOGD(TAG, "Received message: %s", hexencode(message, 121).c_str());
        ...
        void SmartMeterSensorComponent::init_vector() {
            ...
            for (unsigned int i = 0; i < 90; i++) {
                vector_sm.ciphertext[i] = message[i+28];
            }

            // i took this value from the gurux director tool
            byte AuthData[] = {0x62, 0xf9, 0x44, 0x5b, 0x06, 0x9b, 0xa8, 0x66, 0x06, 0x8b, 0x80, 0x36, 0xf2, 0x61, 0x20, 0x30};
            for (int i = 0; i < 16; i++) {
                vector_sm.authdata[i] = AuthData[i];
            }

            for (int i = 0; i < 8; i++) {
                vector_sm.iv[i] = message[14+i]; // manufacturer + serialnumber 8 bytes
            }
            for (int i = 8; i < 12; i++) {
                vector_sm.iv[i] = message[16+i]; // frame counter
            }
        ...
        bool SmartMeterSensorComponent::get_local_time() {
            if(!this->getLocalTime(&ntpTime, 1000)){
